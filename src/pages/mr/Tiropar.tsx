...
const Tiropar = () => {
    ...
    const [data, setData] = useState<{ tiempo: number, posXExp: number, posXTeo: number, posYExp: number, posYTeo: number, velXExp: number, velXTeo: number, velYExp: number, velYTeo: number }[]>([]);
    const [colDefs, setColDefs] = useState<{ headerName?: string; field: "tiempo" | "posXExp" | "posXTeo" | "posYExp" | "posYTeo" | "velXExp" | "velXTeo" | "velYExp" | "velYTeo" }[]>([
        { field: "tiempo" },
        { field: "posXExp" },
        { field: "posXTeo" },
        { field: "posYExp" },
    ...
    function trayectoria() {
        ...
        const newData: { tiempo: number, posXExp: number, posXTeo: number, posYExp: number, posYTeo: number, velXExp: number, velXTeo: number, velYExp: number, velYTeo: number}[] = []
        for (let i = 0; i< N; i++) {
            newData.push({ "tiempo": tiempo[i], "posXExp": posXExp[i], "posXTeo": posXTeo[i], "posYExp": posYExp[i], "posYTeo": posYTeo[i], "velXExp": velXExp[i], "velXTeo": velXTeo[i], "velYExp": velYExp[i], "velYTeo": velYTeo[i] });
        }
    ...
    return(
        ...
        <Layout>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', flexDirection: 'column', alignItems: 'center' }}>
                <div style={{ display: 'flex', gap: 160, flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <div style={{ flex: '1 1 320px', maxWidth: '33%' }}>
                        <Card>
                                <CardHeader>
                                    <CardTitle>Datos Iniciales</CardTitle>
                                    <CardDescription>Introduce los valores iniciales para el cálculo del tiro parabólico.</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <Label>Velocidad Inicial (m/s): </Label>
                                    <Input style={{ width: "150px" }}  placeholder="Velocidad Inicial" value={velI} onChange={e => setVelI(Number(e.target.value))} />
                                </CardContent>
                                <CardContent>
                                    <Label>Masa(kg): </Label>
                                ...
                                <CardContent>
                                    ...
                                    <Input style={{ width: "150px" }}  placeholder="Posición Y Inicial" value={yIni} onChange={e => setYIni(Number(e.target.value))}/>
                                </CardContent>
                                <CardContent>
                                    <Label>Ángulo (°): </Label>
                                    <Input style={{ width: "150px" }}  placeholder="Ángulo" value={ang} onChange={e => setAng(Number(e.target.value) * 180 / Math.PI)}/>
                                </CardContent>
                                <CardContent>
                        ...
                        <div style={{ flex: '1 1 320px', maxWidth: '35%' }}>
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Los resultados más relevantes del lanzamiento.</CardDescription>
